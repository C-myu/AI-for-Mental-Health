...
def render_taxonomy(
    ...
    labels: dict[str, str],
    language: str,
    visible_tag_ids: set[str] | None = None,
) -> list[str]:
    taxonomy = as_dict(config.get("taxonomy"))
    priorities = as_list(taxonomy.get("priorities"))
    tags = [
        tag
        for tag in as_list(taxonomy.get("tags"))
        if visible_tag_ids is None or tag.get("id") in visible_tag_ids
    ]
...
def render_catalog_section(config: dict[str, Any], data: dict[str, Any], language: str) -> str:
    ...
    lines.extend(render_taxonomy(config, labels, language, public_tag_ids))
